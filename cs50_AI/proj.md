...
### Nim
Results for ai50/projects/2024/x/nim generated by check50 v3.3.11
:) nim.py exists
:) nim.py imports
...
可能需要注意的是下面的东西，虽然不是交给我们自己来写的东西。其实有点没看懂，期望以后学习强化学习的时候掌握清楚。
```python
def train(n):
    """
    ...
    for i in range(n):
        ...
        while True:
            ...
            # 如果刚刚下的那一步能够让1号赢，但是现在玩家切换到0号了
            if game.winner is not None:
                # 那么目前这一步对于0号来说，是需要避免的
                player.update(state, action, new_state, -1)
                # 那么目前这一步对于1号来说，是需要肯定的
                player.update(
            ...
            # 如果0号查看state发现是0，那就说明本身这一次实验就有一些问题？
            elif last[game.player]["state"] is not None:
                # 既然还能继续，就不需要额外为reward更新
                player.update(
                    last[game.player]["state"],
                    last[game.player]["action"],
                    new_state,
...
```
## Project 6
这个实验确实非常体现神经网络的玄学，至于怎样能够调节出相对较好的效果，似乎完全需要通过你自己加层数，改层数，使用flatten等奇妙方法，然后再对效果做评估来达到。

感觉多少有一些经验主义了，尤其是在前面还是后面来放置某些操作。

最后的解决方法是：调整了老半天效果不好，但是增加了一个卷积层，一下子就搞定了。。。

Results for ai50/projects/2024/x/traffic generated by check50 v3.3.11
:) traffic.py exists
:) traffic.py imports
...
### Parser
一个类似编译器的实验，这可能是传统的NLP做的东西，但是到后面就不一样了，慢慢演变成了Attention的结构。

在后面做NP_chunk的时候可以用递归来做，很方便。

简单过一遍就行了。
### Attention
大概明白了为什么BERT中会有[MASK]这种东西存在，以及BERT的多个Layer的多个head都是来干什么的，人们可能会通过注意力机制来检查这些head所对应的思考方式。

这一步其实还是多少带着猜测的意思，或许这也是为啥深度学习为啥玄学。

总而言之，CS50-AI我们是真的通关了。虽然这个课比较轻松，但是其实收获还是蛮大的！
