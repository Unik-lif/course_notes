...
- 用户提供函数，云端提供运行环境和部分运维
- 特点：用户代码执行的时间非常短

传统的以虚拟机服务器为单位的云部署方式
- 虚拟机服务器长时间运行，而用户代码执行的时间很短，这并不匹配

新兴的无服务部署模式: serverless
- 只在请求来时，新建服务器实例执行相关逻辑，完成之后就马上销毁实例
- 特点: 由请求驱动，弹性伸缩，按量计费

FaaS特点
- workload需要是Stateless的，运行时间比较短
- 性能指标: 启动时间和运行密度

Serverless使用的执行环境
- 现有方案是虚拟机，动态新建并且销毁一个虚拟机
- 问题是过于重量级，启动时间长，运行密度低，主要的问题是: 虚拟机与宿主机的内核功能有重复

### 方案与改进 - chrrot和namespace
文件系统视图的隔离: chroot，改变当前进程的根目录
- 进程只能从根目录向下开始查找文件

简单设计
- 内核为每个用户记录一个根目录
- 进程打开文件时内核从该用户的根目录开始查找
- chroot对于"/.."与"/"等价，无法通过".."打破隔离，在TCB都指向一个root目录
- 需要root权限才能变更目录

Namespace的限制
- 通过文件系统的name space来限制用户，不允许用户直接用inode访问文件，要求只能通过fd来访问文件
- 当然其他层也可以用于限制用户，但是有可能让namespace失效

但是只有chroot，并不能实现彻底的隔离，因为还有许多资源被共享

### 最佳实践 - linux container
LinuX Container (LXC)
- 由Linux内核提供资源隔离机制
- 由Linux namespace进行安全隔离
- 由Linux cgroup进行性能隔离
...
Mount Namepsace
- 容器内外想要部分共享文件系统
- 通过namespace设置，在内核记录每个NS对于挂载点的修改情况，访问挂载点时，内核需要根据当前的NS的记录来查找文件
- 新的NS会拷贝一份父NS的文件系统树，每个NS都会有独立的文件系统树，修改挂载点只会反映到自己NS的文件系统树上

IPC Namespace
- 共享IPC对象，shared memory的共享与隔离
- 使得每个IPC对象只能属于一个NS
- 每个NS单独记录属于自己的IPC对象
- 进程只能在当前NS中寻找IPC对象
...
Network Namespace
- 基于原本为多VM的机制扩展出来的
- 每个NS拥有一套独立的网络资源，包括IP和网络设备，默认每个只有一个loopback设备
- 建立流程: 创建相连的veth虚拟设备对，该设备对的其中一端加入NS即可联通网络，分配IP后可分别与外界通信

PID Namespace
- 单向的隔离，外部能够看到内部的进程，反之则不能
- 子NS中的进程在父NS中也有PID
- 进程只能看到当前NS的PID
- 还需要做一个映射的关系，Child NS的PID放在Parent NS的PID是存在一定差异的

User Namespace
- 避免容器Root对系统整体有权限
- 对NS内外的UID和GID进行映射，可以基于Linux Capability，允许普通用户在容器内拥有更高的权限
- 但是容器内root用户在容器外并没有特权，只是一个普通用户

Namespace是一个不太漂亮的设计实现，比较锁碎，当然这可能是建立在成熟系统上才决定这么做，模块本身之间的联系也比较繁杂，远比虚拟机本身复杂，但是快
